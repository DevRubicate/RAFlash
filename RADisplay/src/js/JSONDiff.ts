/**
 * JSONDiff - State Synchronization System
 * ========================================
 *
 * This module handles state synchronization between the server (RAEngine) and multiple
 * clients (browser windows, Flash player). It uses a diff-based approach where only
 * changes are transmitted, not full state.
 *
 * ## Architecture Overview
 *
 * ```
 * Client A (browser)                Server (RAEngine)                Client B (browser)
 *       |                                 |                                |
 *       |-- editData(clientDiff) -------->|                                |
 *       |                                 |                                |
 *       |                    processIncomingDiff()                         |
 *       |                    - applies clientDiff                          |
 *       |                    - triggers watchers (derived fields)          |
 *       |                    - returns { fullDiff, derivedDiff }           |
 *       |                                 |                                |
 *       |<-- editData(derivedDiff) -------|                                |
 *       |    (SELF: only watcher changes) |                                |
 *       |                                 |-- editData(fullDiff) --------->|
 *       |                                    (ALL: complete change)        |
 * ```
 *
 * ## Why Two Diffs?
 *
 * - `fullDiff`: The complete net change (client changes + watcher-derived changes).
 *   Sent to OTHER clients who need the full picture.
 *
 * - `derivedDiff`: Only the watcher-generated changes. Sent back to the SOURCE client
 *   because they already have their own changes - they only need the derived fields
 *   that watchers computed (e.g., compiled formulas).
 *
 * ## Watchers
 *
 * Watchers are callbacks that auto-compute derived fields when specific paths change.
 * Example: When `addressA` changes, a watcher computes `compiledA` automatically.
 * This keeps derived data in sync without requiring clients to compute it themselves.
 */
...
export class JSONDiff {
    ...
    /**
     * Processes an incoming diff, applies it, triggers watchers, and calculates the resulting changes.
     *
     * @param target - The object to apply the diff to (mutated in place).
     * @param clientDiff - The initial diff from the client.
     * @returns An object containing:
     *   - `fullDiff`: The complete net change from original state to final state.
     *     This includes both client changes AND watcher-derived changes.
     *     Broadcast to OTHER clients who need the full picture.
    ...
    static processIncomingDiff(target: Record<string, any>, clientDiff: Diff, cloneFn?: (obj: any) => any): { fullDiff: Diff; derivedDiff: Diff } {
        ...
        const fullDiff = this.getDataDiff(stateBefore, target);
        const derivedDiff = this.getDataDiff(stateAfterClientChanges, target);
        return { fullDiff, derivedDiff };
    }
    ...
    static unwatch(pattern: string, callback: (segments: any[]) => void): void {
        this.watchers = this.watchers.filter(w => w.pattern !== pattern || w.callback !== callback);
    }
