/**
 * Deno Test Server for AS2 Unit Tests
 *
 * Serves the test SWF via HTTP and receives test results via XMLSocket.
...
// Flash socket policy file - required for XMLSocket connections
...
async function handleConnection(
    ...
    try {
        ...
        // Check for policy file request (Flash sends this before XMLSocket connect)
        ...
        while (true) {
            ...
            while (buffer.includes("\0")) {
                ...
                const messageStr = buffer.substring(0, nullIndex);
...
async function runTestServer(swfPath: string): Promise<number> {
    ...
    console.log(`${colors.cyan}AS2 Test Server listening on port ${PORT}${colors.reset}`);
