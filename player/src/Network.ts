...
export class Network {
        ...
        Network.http = Deno.serve(
            ...
            async (req: Request): Promise<Response> => {
                ...
                if(url.pathname === '/ws') {
                    ...
                    Network.clients.set(clientId, {type: 'BROWSER', name: null, ws: socket, currentMessageId: 0, pending: []});
                    ...
                    socket.onmessage = async (event) => {
                        ...
                        if(data[0] === 'SETUP') {
                            ...
                            if(typeof data[1].name !== 'string') {
                                throw new Error('Network.onmessage: Invalid SETUP name');
                            }
                            if(data[1].name === 'BROWSER' || data[1].name === 'FLASH') {
                                throw new Error('Network.onmessage: Invalid SETUP name');
                            }
                            Network.clients.get(clientId)!.name = data[1].name;
                            socket.send(JSON.stringify(['SETUP']));
                        } else if(data[0] === 'REQUEST') {
                            const response = await API.handle('ws', data[2]);
                            ...
                            throw new Error(`Network.onmessage: Invalid message type: ${data[0]}`);
                        }
                    };
                    return response;
                } else {
    ...
    static send(target: 'BROWSER' | 'FLASH' | 'ALL' | string, message:any):Promise<Array<any>> {
        ...
        for(let i=0; i<clients.length; ++i) {
            ...
            if(target === 'ALL' || client.type === target || client.name === target) {
                let resolve;
