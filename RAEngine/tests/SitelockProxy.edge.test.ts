...
Deno.test("proxy - first matching active rule wins", async () => {
    const rules = [
...
Deno.test("proxy - mixed text and file rules on different URLs", async () => {
    ...
    try {
        ...
        assertEquals(res1.body, '{"key":"val"}');

        // File rule
        const raw2 = await proxyRequest("GET", "http://cdn.example.com/asset.swf");
        const res2 = parseResponse(raw2);
        assertEquals(res2.status, 200);
        assertEquals(res2.headers["content-type"], "application/x-shockwave-flash");
    } finally {
        stopSitelockProxy();
    }
});
...
Deno.test("proxy - 403 forbidden response", async () => {
    const rules = [
    ...
    try {
        ...
        assertEquals(res.body, "blocked");
    } finally {
        stopSitelockProxy();
    }
});

Deno.test("proxy - 301 redirect status", async () => {
    const rules = [
...
Deno.test("proxy - different unknown hosts all return 404", async () => {
    startSitelockProxy({
        port: TEST_PORT,
        flashPort: 0,
        gameDomain: null,
    ...
    try {
        ...
        for (const url of hosts) {
            ...
            const res = parseResponse(raw);
            assertEquals(res.status, 404, `Expected 404 for ${url}`);
        }
    } finally {
        stopSitelockProxy();
    }
...
Deno.test("proxy - text rule with empty body", async () => {
    const rules = [
...
Deno.test("proxy - JSON file served with correct content type", async () => {
...
Deno.test("proxy - XML file served with correct content type", async () => {
    ...
    try {
        ...
        const res = parseResponse(raw);
        assertEquals(res.headers["content-type"], "application/xml");
    } finally {
        stopSitelockProxy();
    }
});
