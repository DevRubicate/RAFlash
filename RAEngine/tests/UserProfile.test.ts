...
test("UserProfile.getUnlockedIds - returns empty array for game with no unlocks", () => {
    setupProfile();
...
test("recordUnlock then getUnlockedIds returns recorded IDs", () => {
    setupProfile();
    UserProfile.recordUnlock("hash1", 10);
    UserProfile.recordUnlock("hash1", 20);
    UserProfile.recordUnlock("hash1", 30);

    assertEqual(UserProfile.getUnlockedIds("hash1"), [10, 20, 30]);
    assertEqual(UserProfile.getUnlockedIds("hash2"), []);
    cleanup();
});
...
test("dirty flag is false after setup", () => {
    setupProfile();
    assertEqual(UserProfile.dirty, false);
    cleanup();
});

test("dirty flag set to true only on new unlock", () => {
    setupProfile();
    UserProfile.recordUnlock("game1", 1);
    assertEqual(UserProfile.dirty, true);
    ...
    UserProfile.dirty = false;
    UserProfile.recordUnlock("game1", 1);
    assertEqual(UserProfile.dirty, false); // duplicate, no change
    cleanup();
});
