...
export class JSONDiff {
     ...
     * @returns An object containing the total net change (`fullDiff`) and watcher-specific changes (`derivedDiff`).
     */
     ...
     */
    static watch(pattern: string, callback: (segments: unknown[]) => void): void {
        this.watchers.push({ pattern, callback });
    }

    /**
     * Removes a previously registered watcher by matching both pattern and callback reference.
     * @param pattern - The exact pattern string used when registering.
     * @param callback - The exact callback function reference used when registering.
     */
    static unwatch(pattern: string, callback: (segments: unknown[]) => void): void {
